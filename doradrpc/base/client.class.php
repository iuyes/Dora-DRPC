...
class Client
{
    ...
    public function getStat($ip = "", $port = "")
    {
        ...
        $Packet = array(
            ...
            'type' => \DoraDRPC\Base\DoraConst::SW_CONTROL_CMD,
        );

        $sendData = \DoraDRPC\Base\Packet::packEncode($Packet);
        $result = $this->doRequest($sendData, $ip, $port);

        if ($result["code"] == "0" && $guid != $result["data"]["guid"]) {
            return \DoraDRPC\Base\Packet::packFormat("guid wront please retry..", 100100, $result);
        }
    ...
    public function singleAPI($name, $param, $sync = true, $retry = 0, $ip = "", $port = "")
    {
        ...
        if ($sync) {
            $Packet["type"] = \DoraDRPC\Base\DoraConst::SW_SYNC_SINGLE;
        } else {
            $Packet["type"] = \DoraDRPC\Base\DoraConst::SW_ASYNC_SINGLE;
        }
    ...
    public function multiAPI($params, $sync = true, $retry = 0, $ip = "", $port = "")
    {
        ...
        if ($sync) {
            $Packet["type"] = \DoraDRPC\Base\DoraConst::SW_SYNC_MULTI;
        } else {
            $Packet["type"] = \DoraDRPC\Base\DoraConst::SW_ASYNC_MULTI;
        }
        ...
        if ($result["code"] == "0" && $guid != $result["data"]["guid"]) {
            return \DoraDRPC\Base\Packet::packFormat("guid wrong please retry..", 100008, $result);
        }
